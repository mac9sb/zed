...
impl MarkdownPreviewView {
    ...
    fn load_and_display_markdown_file(
        ...
        self.base_directory = path.parent().map(|p| p.to_path_buf());

        self.pending_update_task = Some(cx.spawn_in(window, async move |view, cx| {
            let contents = cx
            ...
            view.update(cx, move |view, cx| {
                view.markdown.update(cx, |markdown, cx| {
                    markdown.reset(SharedString::from(contents), cx);
                });
                view.pending_update_task = None;
                cx.notify();
            })
    ...
    if let Some(path) = resolve_preview_path(url.as_ref(), base_directory.as_deref()) {
        ...
        match behavior {
            ...
            MarkdownPreviewLinkClickBehavior::Preview => {
                open_markdown_preview_for_path(path, workspace, window, cx);
            }
